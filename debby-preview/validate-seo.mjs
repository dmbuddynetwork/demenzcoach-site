...
const retiredGuides = [
  {
    slug: "demenz-wiederholt-fragen",
    campaignToken: "seo_de_wiederholte_fragen"
  },
  {
    slug: "demenz-unruhe-am-abend",
    campaignToken: "seo_de_unruhe_abend"
  },
  {
    slug: "ploetzliche-verwirrtheit-demenz",
    campaignToken: "seo_de_akute_verwirrtheit"
  }
];

for (const guide of retiredGuides) {
  const retiredPath = path.join(root, "ratgeber", `${guide.slug}.html`);
  if (fs.existsSync(retiredPath)) {
    fail(`ratgeber/${guide.slug}.html: retired guide must not be generated`);
  }
  for (const file of htmlFiles) {
    const html = read(file);
    if (html.includes(guide.slug) || html.includes(guide.campaignToken)) {
      fail(`${file}: references retired guide ${guide.slug}`);
    }
  }
}
...
for (const file of ["seo-pages.json", "seo-campaigns.json", "sitemap.xml"]) {
  const content = read(file);
  for (const guide of retiredGuides) {
    if (content.includes(guide.slug) || content.includes(guide.campaignToken)) {
      fail(`${file}: references retired guide ${guide.slug}`);
    }
  }
}
